...
const deletegamepost = async (req, res) => {
  ...
  try {
    ...
    res.status(200).send({ message: t('deletegamepost.content1') });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};
...
const updateUpcomingGamesBlog = async (req, res) => {
  const { postId, title, subtitle, content, cover_image } = req.body;
...
const createnewspost = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: transla('createnewspost.content1')});
  } catch (e) {
    await t.rollback();
  }
};
  ...
  try {
    ...
    if (needsUpdate) {
      try {
        ...
        return res.status(200).json({ message: t('updateNewsBlog.content1') });
      } catch (error) {
        await t1.rollback();

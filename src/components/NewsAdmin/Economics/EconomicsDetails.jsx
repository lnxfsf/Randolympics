...
const EconomicsDetails = ({ postZ, onBack }) => {
  ...
  const deletePost = async () => {
    try {
      const response = await axios.post(
        `${BACKEND_SERVER_BASE_URL}/blog/deleteeconomicspost`,
        {
          postId: post.postId,
        }
      );
  ...
  const handleCancel = () => {
    ...
    // it must delete that image from server that was temporary uploaded, but not yet saved to that blog post. YES, BUT ONLY IF IT'S UPLOADED IN FIRST PLACE ! 
    if(tempEditCoverImage){
    fetch(
      `${BACKEND_SERVER_BASE_URL}/imageUpload/revertBlogs_economics_picture_upload`,
      {
        method: "DELETE",
        headers: {
...
{/* 
            <img
              className="blogControlsIcon cursor-pointer"
              src={editingImage}
